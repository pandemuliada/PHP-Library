                  ...
                  <div class="row">
                    ...
                    <?php if (isset($_SESSION['success_msg'])) : ?>
                      <div class="alert alert-success">
                        <?= $_SESSION['success_msg'] ?> <a href="list_pinjam.php" class="alert-link">Lihat Hasil</a>
                      </div>
                    <?php endif; ?>
                        ...
                        <div class="form-group">
                          ...
                          <select name="anggota" class="form-control mb-4">
                            <?php foreach ($data_anggota as $anggota) : ?>
                              <option value="<?= $anggota['id_anggota'] ?>" <?= $anggota['id_anggota'] == $data_pinjam['id_anggota'] ? 'selected' : '' ?>><?= $anggota['nama_anggota'] ?></option>
                            <?php endforeach; ?>
                          </select>
                        </div>
                        <div class="form-group">
                          ...
                          <select name="buku" class="form-control mb-4">
                            <?php foreach ($data_buku as $buku) : ?>
                              <option value="<?= $buku['id_buku'] ?>" <?= $buku['id_buku'] == $data_pinjam['id_buku'] ? 'selected' : '' ?>><?= $buku['judul_buku'] ?></option>
                            <?php endforeach; ?>
                          </select>
                        </div>
                        <div class="form-group">
                          <label>Tanggal Pinjam</label>
                          <input type="date" name="tgl_pinjam" value="<?= $data_pinjam['tgl_pinjam'] ?>" class="form-control mb-4" placeholder="Tanggal Pinjam">
                        </div>
                        <div class="form-group">
                          <label>Tanggal Jatuh Tempo</label>
                          <input type="date" name="tgl_jatuh_tempo" value="<?= $data_pinjam['tgl_jatuh_tempo'] ?>" class="form-control mb-4" placeholder="Tanggal Jatuh Tempo">
                        </div>
                        <input type="submit" name="submit" class="btn btn-gradient-success btn-block my-4" value="Edit Data Peminjaman">
                      </form>
                      <!-- END FORM -->
                    </div>
                  </div>
